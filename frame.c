...
static void rx_start(void) {
  ...
  EICRA &= ~( 1<<GDO2_INT_ISCn1 | 1<<GDO2_INT_ISCn0 );
  EICRA |=  ( 0<<GDO2_INT_ISCn1 | 1<<GDO2_INT_ISCn0 );  // rising and falling edge
...
static void tx_start(void) {
  ...
  // Make sure configured as input in case shared with RX
  GDO2_DDR  &= ~GDO2_IN;
  GDO2_PORT |=  GDO2_IN;         // Set input pull-up

  // Make sure configured as output in case shared with RX
  GDO0_DDR  |=  GDO0_IN;
  GDO0_PORT |=  GDO0_IN;         // Set input pull-up

  EICRA &= ~( 1<<GDO2_INT_ISCn1 | 1<<GDO2_INT_ISCn0 );
  EICRA |=  ( 1<<GDO2_INT_ISCn1 | 0<<GDO2_INT_ISCn0 );  // Falling edge
...
ISR(GDO2_INT_VECT) {
  DEBUG_ISR(1);
  switch( radioMode ) {
  case RADIO_RX:    isr_rx();   break;
  case RADIO_TX:    isr_tx();   break;
...
static void frame_rx_disable(void) {
  uint8_t sreg = SREG;
  cli();
...
static void frame_tx_disable(void) {
  uint8_t sreg = SREG;
  cli();
